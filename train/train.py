...
def load_data() -> pd.DataFrame:
    ...
    conn.close()
    print(f"  Loaded {len(df):,} rows (14-day trend + historical replay buffer).")
    return df
...
def finetune(df: pd.DataFrame):
    ...
    print("Loading preprocessor from Hugging Face...")
    ...
    # Gentle learning rate (0.0001) for fine-tuning
    model.compile(
    ...
    print(f"\n  MAE: {mae:,.0f} AED | R²:  {r2:.4f}")

    metrics = {
        "mae": round(float(mae), 2), "r2": round(float(r2), 4),
        "train_rows": int(len(X_train)), "test_rows": int(len(X_test)),
        "trained_at": datetime.utcnow().isoformat(),
    }
    return model, preprocessor, metrics

#5.Push updated model to Hugging face
...
if __name__ == "__main__":
    ...
    push_to_hf(model, preprocessor, metrics)
    print("\nFine-tune complete.")